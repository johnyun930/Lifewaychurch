...
.post((req:Request,res:Response)=>{
    ...
    BulletenBoard.find((err,docs)=>{
        ...
        const post: IPost = new BulletenBoard({
            id,
            title,
            bibleText,
            date,
...
router.route('/:id').
get((req:Request,res:Response)=>{
    BulletenBoard.findById(req.params.id,(err:Error,doc:IPost)=>{
        res.send(doc);
    });
})
.delete((req:Request,res:Response)=>{
    BulletenBoard.findByIdAndDelete(req.params.id).then(()=>{
